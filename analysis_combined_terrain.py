...
for index, i in enumerate(CRR):
    for index2, j in enumerate(SLOPE):
        ...
        terrain_angle = numpy.array([terrain_num]*int(len(j)))   # Angle value repeated in array of same size as omega
        omegaLow = numpy.array(([0.0]*int(len(j))))   # low omega repeated in array same size length as slope_array_deg
        omegaHigh = numpy.array(([3.8]*int(len(j))))   # high omega in array same size as slope_array_deg
