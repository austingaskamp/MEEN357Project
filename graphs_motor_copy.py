...
torque_out = torque * get_gear_ratio(rover['wheel_assembly']['speed_reducer'])
...
g1.plot(omega_out, torque_out, '-b')
g1.set_xlabel('Motor Shaft Torque (N*m)')
g1.set_ylabel('Motor Shaft Speed (rad/s)')
g1.grid(True)

x = arange(0, 170,.5)
# y = -(rover['wheel_assembly']['motor']['speed_noload'] / rover['wheel_assembly']['motor']['torque_stall']) * torque_out**2 + rover['wheel_assembly']['motor']['speed_noload'] * torque_out
y = -(rover['wheel_assembly']['motor']['speed_noload'] / rover['wheel_assembly']['motor']['torque_stall'])*torque_out**2 + rover['wheel_assembly']['motor']['speed_noload']*torque_out / get_gear_ratio(rover['wheel_assembly']['speed_reducer'])

g2.plot(torque_out, y, '-b')
g2.set_xlabel('Motor Shaft Torque (N*m)')
g2.set_ylabel('Motor Power (W)')
g2.grid(True)
